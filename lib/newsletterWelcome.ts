 ...
 * the Kit "Welcome" sequence (id 2873722) — copy and the "WELCOME N/5"
 * banner art both. Email N goes out `WELCOME_SCHEDULE[N-1]` days after
 * Subscribed Date; email 1 is sent immediately on signup.
 *
 ...
 *  Matches Kit's cumulative delays: 0, +2, +3, +3, +3. */
...
/** "WELCOME N/5" banner (Asphalt & Dirt branding is baked into the art), then
 *  the white content area. */
function open(step: number) {
  return `<img src="${SITE_URL}/img/newsletter/welcome-${step}.jpg" width="600" alt="Asphalt &amp; Dirt — Welcome ${step} of 5" style="display:block;width:100%;max-width:600px;height:auto;border:0;border-radius:8px 8px 0 0;">
  <div style="background:#ffffff;padding:28px 28px 8px;">`;
}
function close() {
...
const EMAILS: Record<number, WelcomeBody> = {
  1: {
...
export function buildWelcomeEmail(step: number): NewsletterContent {
  ...
  return {
    ...
    innerHtml: open(step) + email.body + close(),
  };
}

...
/** Builds the Kit v4 broadcast payload for a single blog post — a quick
 *  "we just published this" announcement, distinct from the weekly digest
 *  below. Pure formatting — no AI pass here on purpose: the newsletter
 *  should be an exact, reliable reflection of what's actually live on the
 *  site, not a re-generated approximation of it. */
export function buildBroadcastPayload(post: BlogPost) {
  ...
  return {
    ...
    content,
    description: `Newsletter for blog post: ${post.title}`,
    public: false,
    published_at: new Date().toISOString(),
    send_at: null, // draft — reviewed and sent manually in Kit
    thumbnail_url: imageUrl,
    thumbnail_alt: post.image.alt,
    subscriber_filter: [],
  };
}
    ...
    <div style="background:${BLACK};padding:28px 24px;border-radius:8px 8px 0 0;">
      <table width="100%" style="border-collapse:collapse;">
        <tr>
          ...
          <td style="vertical-align:middle;color:${GRAY};font-size:13px;letter-spacing:0.06em;text-transform:uppercase;text-align:right;">Weekly</td>
        </tr>
      </table>
    </div>
...
/**
 * Builds the Kit v4 broadcast payload for the weekly digest — feature story,
 * a build spotlight, a gear verdict, what's coming up, an optional rig
 * spotlight, quick hits, and a closing community CTA. Everything with a real
 * site data source is pulled automatically; `trailTalk` and `rigOfTheWeek`
 * have no such source and are only included when explicitly passed in.
 *
 * This never sends — same as buildBroadcastPayload, the result is a Kit
 * draft (send_at: null) for a human to review, trim sections from, and send
 * by hand.
 */
export async function buildWeeklyDigestPayload(options: WeeklyDigestOptions = {}) {
  ...
  const subject = featureStory ? `This Week: ${featureStory.title}` : "This Week In Asphalt & Dirt";
  const previewText = featureStory?.excerpt || "Builds, trails, gear, and community — this week's roundup.";

  return {
    subject,
    preview_text: previewText,
    content,
    description: "Weekly Asphalt & Dirt digest",
    public: false,
    published_at: new Date().toISOString(),
    send_at: null, // draft — reviewed, trimmed, and sent manually in Kit
    thumbnail_url: featureStory ? absoluteUrl(featureStory.image.src) : undefined,
    thumbnail_alt: featureStory?.image.alt,
    subscriber_filter: [],
  };
}
